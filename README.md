...
1. **영상 업로드 · 프레임 추출** (한 단계) — 영상 업로드 후 지정 FPS / 일정 간격(초) / 전체(원본 FPS 지정)로 추출
2. **로고 분리** — 흰 배경을 투명하게. 3가지 방식:
   - `Luminance 매트` (권장): 밝기를 그대로 투명도로. Figma의 Multiply와 동등하되 실제 알파 채널로 추출
   - `Threshold + 페더`: 임계값 + 부드러운 경계
   - `Threshold 하드`: 이진 마스크
3. **배경 적용** — 여러 배경 업로드 후 배치: 순서대로 / 일정 프레임 간격 / 프레임별 직접 지정
4. **미리보기** — 합성 결과를 재생. 재생 FPS·배경 전환 간격 조절, 스크럽.
   화면 비율은 SNS 비율 외에 **A1 포스터(594×841mm 세로/가로)** 지원.
   로고에 **투명도**와 **합성 효과**(multiply / screen / overlay / soft-light 등 Canvas 블렌드 모드) 적용 가능
5. **내보내기** — 프레임 ZIP 일괄 저장(순서 파일명) / WebM 영상. 합성본 또는 로고 투명 PNG 선택
6. **가공본 재조합** — 외부에서 가공한 프레임 업로드 → 파일명 자연 정렬 → 재생 → WebM 내보내기
